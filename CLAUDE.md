...
## 🤖 AI Testing - Website UI Knowledge Base

### 웹사이트별 주요 UI 가이드

이 섹션은 AI 기반 테스팅에서 각 웹사이트의 주요 UI 요소와 동작을 이해하는데 사용됩니다.

#### 쿠팡 (coupang.com)
- **로고 이미지** (보통 왼쪽 상단): 클릭 시 홈페이지로 이동
- **검색창** (상단 중앙): 상품 검색 가능
- **카테고리 메뉴** (좌측 또는 상단): "식품", "패션", "뷰티", "가전" 등
- **상품 카드**: 이미지, 가격, 상품명 포함. 클릭 시 상세 페이지로 이동
- **장바구니 아이콘** (우측 상단): 장바구니 페이지로 이동
- **"로켓배송" 배지**: 빠른 배송 상품 표시
- **"장바구니 담기" 버튼**: 상품 상세 페이지에서 장바구니에 추가
- **수량 조절 버튼**: +/- 버튼으로 수량 변경

#### 네이버 (naver.com)
- **녹색 로고** (왼쪽 상단): 클릭 시 메인 페이지로 이동
- **검색창** (중앙): 통합 검색
- **GNB 메뉴** (상단): 뉴스, 카페, 블로그, 쇼핑 등
- **로그인 버튼** (우측 상단)
- **쇼핑 탭**: 네이버 쇼핑으로 이동
...
- **뒤로가기 버튼**: 이전 페이지로 이동
- **닫기 버튼 (X)**: 팝업이나 모달 닫기
- **하단 고정 버튼**: 주요 액션 (구매, 장바구니 등)
- **상단 헤더**: 로고, 검색, 메뉴, 장바구니 등 주요 네비게이션
- **푸터**: 회사 정보, 이용약관 등

### AI 프롬프트 가이드라인

AI가 웹 테스팅 시 따라야 할 규칙:

1. **미션 우선**: 주어진 미션과 가장 관련 있는 요소를 선택
2. **히스토리 고려**: 이미 클릭한 요소는 피하기
3. **시각적 확인**: 스크린샷에서 확인 가능한 요소 우선
4. **UI 영역 회피**: URL 바 영역(y < 150)은 클릭 금지
5. **콘텐츠 우선**: 광고나 프로모션보다 실제 콘텐츠 우선
6. **웹사이트 지식 활용**: 위 UI 가이드를 참고하여 기능 이해
7. **최적 경로**: 미션 달성을 위한 최단 경로 선택
